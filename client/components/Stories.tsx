...
function ProgressBar({ active, done, paused, duration = 5000 }: {
  ...
  return (
    <div className="h-[2.5px] flex-1 rounded-full overflow-hidden" style={{ background: "var(--separator)" }}>
      <div
  ...
  return (
    <div className="absolute inset-0" style={{ borderRadius: 28, overflow: "hidden" }}>
      ...
      <div className="absolute top-3 left-3 flex items-center gap-2">
        <div className="w-7 h-7 rounded-full overflow-hidden" style={{ border: "2px solid rgba(255,255,255,0.85)" }}>
          <img src={story.avatar} alt="" className="w-full h-full object-cover" />
        </div>
        <p className="text-[11px] font-bold" style={{ color: "var(--text-primary)" }}>{story.name}</p>
      </div>
    </div>
  );
}
...
function StoryViewer({
  ...
  return (
    <div
      ...
      style={{ background: "var(--bg)", opacity: vOpacity }}
    >
      ...
      >
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-[13px] font-bold leading-tight" style={{ color: "var(--text-primary)" }}>{current?.name}</p>
              <p className="text-[11px]" style={{ color: "var(--text-secondary)" }}>@{current?.userId}</p>
            </div>
          </div>
          <button
            className="ios-icon-btn"
            style={{ width: 32, height: 32, background: "rgba(28,28,30,0.7)" }}
        ...
        {dragY > 50 && (
          <div className="absolute inset-0 flex items-end justify-center pb-20 pointer-events-none z-30">
            <div className="px-5 py-2 rounded-full ios-blur" style={{ background: "rgba(28,28,30,0.8)" }}>
              <p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Отпустите, чтобы закрыть</p>
            </div>
          </div>
        )}
      ...
      <div className={`relative w-[68px] h-[68px] rounded-full p-[2.5px] ${user.seen ? 'story-ring-seen' : 'story-ring'}`}>
        ...
        {user.slides.length > 1 && (
          <div className="absolute -bottom-0.5 -right-0.5 w-5 h-5 rounded-full flex items-center justify-center"
            style={{ background: "var(--blue)", border: "2px solid var(--bg)" }}>
            <span className="text-white text-[9px] font-bold">{user.slides.length}</span>
          </div>
        )}
      </div>
      <span className="text-[11px] font-medium text-center w-16 truncate" style={{ color: "var(--text-secondary)" }}>{user.name}</span>
    </button>
  );
}
      ...
      <div className="w-full mb-4 overflow-hidden">
        <div className="flex gap-3 px-4 py-2 overflow-x-auto scrollbar-hide">
          ...
          >
            ...
            <span className="text-[11px] font-medium text-center w-16 truncate" style={{ color: "var(--text-secondary)" }}>История</span>
          </button>
